...
func TestDownloadShards(t *testing.T) {
	...
	t.Run("no enough hosts", func(t *testing.T) {
		_, err := sm.downloadShards(context.Background(), slab, nil, d, zap.NewNop())
	...
	t.Run("no enough hosts", func(t *testing.T) {
		...
		_, err := sm.downloadShards(context.Background(), unavailableSlab, nil, d, zap.NewNop())
		if !errors.Is(err, errNotEnoughShards) {
			t.Fatal(err)
		}
	})
	...
	t.Run("success", func(t *testing.T) {
		sectors, err := sm.downloadShards(context.Background(), slab, allHosts, d, zap.NewNop())
		if err != nil {
			t.Fatal(err)
