...
func (m *SlabManager) migrateSlabs(ctx context.Context, slabIDs []SlabID, d *dialer, log *zap.Logger) error {
	...
	for _, slabID := range slabIDs {
		...
		go func(slabID SlabID) {
			defer wg.Done()
			m.migrateSlab(ctx, slabID, allHosts, goodContracts, ms.Period, d, log)
		}(slabID)
	}
	wg.Wait()
	return nil
...
func (m *SlabManager) migrateSlab(ctx context.Context, slabID SlabID, allHosts []hosts.Host, goodContracts []contracts.Contract, period uint64, d *dialer, log *zap.Logger) {
	start := time.Now()
	...
	// out the database
	shards, err := m.downloadShards(ctx, slab, allHosts, d, log.Named("recover"))
	if err != nil {
	...
	migrated, err := m.uploadShards(ctx, slab, shards, uploadCandidates, d, log.Named("migrate"))
