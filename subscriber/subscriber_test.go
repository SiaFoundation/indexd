...
func TestSyncBatching(t *testing.T) {
	...
	cmm, err := contracts.NewManager(sk, am, nil, cm, store, nil, nil, nil, contracts.NewContractLocker(), hm, s, w, contracts.WithLogger(log.Named("contracts")), contracts.WithSyncPollInterval(250*time.Millisecond))
	if err != nil {
		t.Fatal(err)
	}
	defer cmm.Close()
