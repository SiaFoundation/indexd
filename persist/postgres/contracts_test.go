...
func TestPruneRenewedTo(t *testing.T) {
	store := initPostgres(t, zaptest.NewLogger(t).Named("postgres"))
	...
	contractID := addContract(12)
	childContractID1 := addContract(10)
	childContractID2 := addContract(11)

	// renew parent to first child
	if err := store.UpdateChainState(context.Background(), func(tx subscriber.UpdateTx) error {
		if err := tx.UpdateContractState(contractID, contracts.ContractStateResolved); err != nil {
			return err
		}
	...
	if contract, err := store.Contract(context.Background(), contractID); err != nil {
		t.Fatal(err)
	} else if contract.RenewedTo != childContractID1 {
		t.Fatalf("expected RenewedTo ID %v, got %v", childContractID1, contract.RenewedTo)
	} else if contract.State != contracts.ContractStateResolved {
		t.Fatalf("expected state %v, got %v", contracts.ContractStateResolved, contract.State)
	}

	if contract, err := store.Contract(context.Background(), childContractID1); err != nil {
		t.Fatal(err)
	} else if contract.RenewedTo != (types.FileContractID{}) {
		t.Fatalf("expected child contract not to be renewed")
	} else if contract.State != contracts.ContractStateActive {
		t.Fatalf("expected state %v, got %v", contracts.ContractStateActive, contract.State)
	}

	// renew parent to second child and set first child to pending
	if err := store.UpdateChainState(context.Background(), func(tx subscriber.UpdateTx) error {
		if err := tx.UpdateContractState(contractID, contracts.ContractStateResolved); err != nil {
			return err
		}
		if err := tx.UpdateContractState(childContractID1, contracts.ContractStatePending); err != nil {
			return err
		}
	...
	if contract, err := store.Contract(context.Background(), contractID); err != nil {
		...
		t.Fatalf("expected RenewedTo ID %v, got %v", childContractID2, contract.RenewedTo)
	} else if contract.State != contracts.ContractStateResolved {
		t.Fatalf("expected state %v, got %v", contracts.ContractStateResolved, contract.State)
	}
	...
	if contract, err := store.Contract(context.Background(), childContractID1); err != nil {
		...
		t.Fatalf("expected child contract not to be renewed")
	} else if contract.State != contracts.ContractStatePending {
		t.Fatalf("expected state %v, got %v", contracts.ContractStatePending, contract.State)
	}

	if contract, err := store.Contract(context.Background(), childContractID2); err != nil {
		t.Fatal(err)
	} else if contract.RenewedTo != (types.FileContractID{}) {
		t.Fatalf("expected child contract not to be renewed")
	} else if contract.State != contracts.ContractStateActive {
	...
	// reject and prune first child
	if err := store.PruneExpiredContractElements(context.Background(), 2); err != nil {
		t.Fatal(err)
	} else if err := store.PruneContractSectorsMap(context.Background(), 2); err != nil {
		t.Fatal(err)
	} else if _, err := store.pool.Exec(context.Background(), `UPDATE contracts SET state = $1 WHERE state = $2 AND contract_id = $3`, sqlContractState(contracts.ContractStateRejected), sqlContractState(contracts.ContractStatePending), sqlHash256(childContractID1)); err != nil {
		t.Fatal(err)
	}

	// confirm parent and second child not pruned
	if _, err := store.ContractElement(context.Background(), contractID); err != nil {
		t.Fatal(err)
	} else if _, err := store.ContractElement(context.Background(), childContractID2); err != nil {
		t.Fatal(err)
	}

	// confirm first child is pruned
	if _, err := store.ContractElement(context.Background(), childContractID1); !errors.Is(err, contracts.ErrNotFound) {
	...
	// first child should be rejected
	if contract, err := store.Contract(context.Background(), childContractID1); err != nil {
		t.Fatal(err)
	} else if contract.RenewedTo != (types.FileContractID{}) {
		t.Fatalf("expected child contract not to be renewed")
	} else if contract.State != contracts.ContractStateRejected {
		t.Fatalf("expected state %v, got %v", contracts.ContractStateRejected, contract.State)
	}

	// second child should be active
	if contract, err := store.Contract(context.Background(), childContractID2); err != nil {
		t.Fatal(err)
	} else if contract.RenewedTo != (types.FileContractID{}) {
		t.Fatalf("expected child contract not to be renewed")
