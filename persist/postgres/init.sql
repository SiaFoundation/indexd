...
CREATE TABLE objects (
    ...
    encrypted_data_key BYTEA UNIQUE NOT NULL CHECK(LENGTH(encrypted_master_key) = 72), -- user provided, data encryption key (xchacha20 nonce + key + tag)
    encrypted_meta_key BYTEA UNIQUE NOT NULL CHECK(LENGTH(encrypted_master_key) = 72), -- user provided, metadata encryption key (xchacha20 nonce + key + tag)
    ...
    data_signature BYTEA UNIQUE NOT NULL CHECK(LENGTH(signature) = 64) -- signature of blake2b(object_key || encrypted_data_key)
    meta_signature BYTEA UNIQUE NOT NULL CHECK(LENGTH(signature) = 64) -- signature of blake2b(object ID, metadata key, and encrypted_metadata)
);
