...
func runRootCmd(ctx context.Context, cfg config.Config, walletKey types.PrivateKey, network *consensus.Network, genesis types.Block, log *zap.Logger) error {
	...
	funder := accounts.NewFunder(cm, contracts.NewSigner(walletKey), types.Siacoins(1))
	...
	web := http.Server{
		Handler: webRouter{
			api: jape.BasicAuth(cfg.HTTP.Password)(api.NewServer(cm, contracts, s, wm, store, apiOpts...)),
		},
