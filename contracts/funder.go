...
type (
	...
	// FunderDialer defines an interface for dialing the host and returning a host client. This client can be used to
	// interact with the host using the RHP methods. The client is expected to be closed when no longer needed.
	FunderDialer interface {
		DialHost(ctx context.Context, hostKey types.PublicKey, addrs []chain.NetAddress) (FunderHostClient, error)
	}
...
type funderWrapper struct {
	d *client.Dialer
}

// DialHost dials the host and returns a FunderHostClient.
func (w *funderWrapper) DialHost(ctx context.Context, hostKey types.PublicKey, addrs []chain.NetAddress) (FunderHostClient, error) {
...
func NewFunder(dialer *client.Dialer) *Funder {
	return &Funder{dialer: &funderWrapper{d: dialer}}
}
